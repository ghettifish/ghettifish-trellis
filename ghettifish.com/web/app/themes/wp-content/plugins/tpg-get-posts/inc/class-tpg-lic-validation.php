...
class tpg_lic_validation {
	...
	function update_source(array $_p){
		...
		if (is_wp_error($_tmpflnm)) {
			...
			$_msg=__('Either the connection failed or you have reached your download limit. Load download link directly in browser for addtional information.','tpg-get-posts').' <br>'.__('Download link:','tpg-get-posts').$_p['dl_url'];
			//show_message($_msg);
